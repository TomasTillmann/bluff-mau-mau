...
function actionButton(move) {
  return `<button id="move-${move.type}" type="button" class="bp-button bp-button--${move.type === 'accept' ? 'primary' : 'secondary'} bp-button--wide" data-action="${move.type}" ${busy ? 'disabled' : ''}>${actionLabel(move.type)}</button>`;
}
...
function panel() {
  ...
  const helper = state.winner !== null ? 'The last play is resolved. Deal again for another game.' : responding ? `${playerName(1 - state.turn)} declared ${cardName(state.top)}. Click the face-down discard to challenge. Choosing your next action accepts the declaration.` : 'Choose a card from your hand. Play it as itself, or use the grid to declare another identity.';
  const responseMoves = state.legal_moves.filter(item => ['accept', 'challenge'].includes(item.type));
  return `<aside class="bp-action-panel game-panel" aria-labelledby="actor"><h2 id="actor" class="bp-action-panel__title" tabindex="-1">${title}</h2><p class="bp-action-panel__helper">${helper}</p>${responseMoves.length ? `<div class="bp-actions game-response-actions">${responseMoves.map(actionButton).join('')}</div>` : ''}<div class="bp-field"><h3 class="bp-field__title">Actual card</h3>${actual && state.phase === 'turn' ? `<div class="bp-actual"><div class="bp-card bp-card--mini">${cardImage(actual, cardName(actual))}</div><p class="bp-actual__name">${cardName(actual)}<span class="bp-actual__hint">Selected from ${playerName(state.turn)}’s hand</span></p></div>` : '<p class="game-actual-empty">No card selected for play.</p>'}</div>${declarations()}${needsSuit ? `<div class="game-queen"><h3 class="bp-field__title" id="suit-label">Continue with</h3><div class="game-suits" role="group" aria-labelledby="suit-label">${Object.entries(suits).map(([code, name]) => `<button id="suit-${code}" type="button" class="game-suit" data-suit="${code}" aria-pressed="${chosenSuit === code}" ${busy ? 'disabled' : ''}>${suitMark(code)}<span>${name[0].toUpperCase() + name.slice(1)}</span></button>`).join('')}</div></div>` : ''}<button id="play-truthful" type="button" class="bp-button bp-button--secondary bp-button--wide game-truthful" ${truthfulMove ? `data-move="${truthfulMove.id}"` : ''} ${!truthfulMove || busy ? 'disabled' : ''}>Play ${actual ? `${cardMark(actual)} ` : ''}as itself</button><p class="bp-play-summary">${actual && declared && state.phase === 'turn' ? `Play ${cardMark(actual)} as ${cardMark(declared)}` : 'Choose your declaration'}</p>${declared ? `<p class="bp-penalty">${declarationEffect()}</p>` : ''}${reason ? `<p id="play-reason" class="game-play-reason" data-invalid="${state.phase === 'turn' && !!declared}">${escapeHTML(reason)}</p>` : ''}<div class="bp-actions"><button id="play-card" type="button" class="bp-button bp-button--primary bp-button--wide" ${move ? `data-move="${move.id}"` : ''} ${reason ? 'aria-describedby="play-reason"' : ''} ${!move || busy ? 'disabled' : ''}>Play face down</button>${turnActions().map(actionButton).join('')}</div><section class="bp-history game-history" aria-labelledby="history-title"><h3 id="history-title" class="bp-history__title">Recent moves</h3>${state.history.length ? `<ol class="bp-history__list" reversed>${state.history.slice(-6).reverse().map(item => `<li class="bp-history__row"><span class="bp-history__player">${item.player === null ? 'Table' : playerName(item.player)}</span><span>${escapeHTML(item.text)}</span></li>`).join('')}</ol>` : '<p class="game-history-empty">The cards are dealt. Make the first move.</p>'}</section></aside>`;
}
